...
def run_with_fallback(db: Session, user: User, fn, preferred: str | None = None):
    ...
    Returns (result, provider_id_used).
    Raises HTTPException if all keys fail or none are configured.
    """
    ...
    for row in rows:
        ...
        if not api_key:
            errors.append(
                f"{row.provider}: stored key could not be decrypted"
            )
            continue
        ...
        try:
            ...
            print("\n====================")
            print(f"Provider failed: {row.provider}")
    ...
    raise HTTPException(
        ...
        detail="All configured API keys failed. Details — "
        + " | ".join(errors),
    )